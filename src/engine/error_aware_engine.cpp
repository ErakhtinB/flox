...
#include "flox/engine/error_aware_engine.h"
...
ErrorAwareEngine::ErrorAwareEngine(const EngineConfig& config,
...
VoidResult ErrorAwareEngine::start()
{
...
VoidResult ErrorAwareEngine::stop()
{
...
Result<EngineHealthStatus> ErrorAwareEngine::getHealthStatus() const
{
  std::shared_lock lock(_healthMutex);
  return _healthStatus;
}

VoidResult ErrorAwareEngine::addSubsystem(std::unique_ptr<ISubsystem> subsystem)
{
  if (!subsystem)
  {
...
VoidResult ErrorAwareEngine::addConnector(std::shared_ptr<ExchangeConnector> connector)
{
  if (!connector)
  {
...
Result<std::map<ErrorCategory, size_t>> ErrorAwareEngine::getErrorStatistics() const
{
  std::shared_lock lock(_errorMutex);
  return _errorCounts;
}

void ErrorAwareEngine::reportError(const FloxError& error)
{
...
void ErrorAwareEngine::clearErrors(ErrorCategory category)
{
  std::unique_lock lock(_errorMutex);
...
VoidResult ErrorAwareEngine::executeWithCircuitBreaker(const std::string& operation,
                                                       std::function<VoidResult()> func)
{
  ...
  if (breaker.isOpen.load())
  {
    ...
    if (now - breaker.lastFailureTime < CircuitBreakerState::RECOVERY_TIMEOUT)
    {
  ...
  else
  {
    ...
    if (failures >= CircuitBreakerState::FAILURE_THRESHOLD)
    {
...
VoidResult ErrorAwareEngine::validateEngineState() const
{
...
VoidResult ErrorAwareEngine::startSubsystems()
{
...
VoidResult ErrorAwareEngine::startConnectors()
{
...
VoidResult ErrorAwareEngine::stopSubsystems()
{
  VoidResult result;
...
VoidResult ErrorAwareEngine::stopConnectors()
{
  VoidResult result;
...
void ErrorAwareEngine::updateHealthStatus()
{
